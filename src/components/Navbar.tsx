...
export const Navbar: React.FC<NavbarProps> = ({ onOpenGuidelines, onOpenGallery }) => {
  ...
  const handleNavClick = (e: React.MouseEvent<HTMLAnchorElement>, href: string, id: string) => {
    ...
    if (id === 'gallery' && onOpenGallery) {
      // scroll to gallery preview or trigger
        ...
        <div className="lg:hidden border-t border-gray-100 bg-white px-4 pt-3 pb-6 space-y-2 shadow-lg animate-fadeIn">
            ...
            {NAV_ITEMS.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {isActive && <span className="w-1.5 h-1.5 rounded-full bg-[#D91E27]" />}
                </a>
              );
            })}
          </div>
          ...
          <div className="pt-4 border-t border-gray-100 space-y-2">
            <div className="p-3 bg-[#F9F9F9] rounded-sm text-xs text-[#636E72] space-y-1 border border-gray-100">
              <p className="font-bold text-[#0A3D62] uppercase tracking-wider text-[11px]">Rasheed Hospital & Dental Complex</p>
              <p className="text-[11px]">{HOSPITAL_INFO.address}, Multan</p>
              <p className="text-[#D91E27] font-semibold text-[11px]">061-6560560 / 0328-6560560</p>
            </div>
